...
const InvoiceModal = (props) => {
  ...
  const handleSubmit = async () => {
    try {
      await props.submitHandler(props.info);
      await GenerateInvoice(props.info);
      props.closeModal();
      navigate("/");
    } catch (err) {
    ...
    <div>
      <Modal show={props.showModal} onHide={props.closeModal} size="lg" centered>
          ...
          <div className="p-4">
            ...
            <Table>
              <tbody>
                ...
                {props.taxAmount !== 0.00 &&
                  <tr className="text-end">
                    <td></td>
                    <td className="fw-bold" style={{ width: '100px' }}>TAX</td>
                    <td className="text-end" style={{ width: '100px' }}>{props.currency} {props.taxAmount}</td>
                  </tr>
                }
                {props.discountAmount !== 0.00 &&
                  <tr className="text-end">
                    ...
                    <td className="text-end" style={{ width: '100px' }}>{props.currency} {props.discountAmount}</td>
                  </tr>
                }
                <tr className="text-end">
                  <td></td>
        ...
        <div className="pb-4 px-4">
          <Row>
            <Col md={6}>
              <Button variant="primary" className="d-block w-100" onClick={handleSubmit}>
                <BiPaperPlane style={{ width: '15px', height: '15px', marginTop: '-3px' }} className="me-2" />Send Invoice
              </Button>
            </Col>
            <Col md={6}>
              <Button variant="outline-primary" className="d-block w-100 mt-3 mt-md-0" onClick={handleSubmit}>
